...
for year in range(2012, 2024):
    ...
    url = f' https://www.basketball-reference.com/teams/NYK/{year}_games.html'
    ...
    all_data_NBA_NYK.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NBA_NYK = pd.concat(all_data_NBA_NYK, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NBA_NYK.head())

#export final_df to csv
final_df_NBA_NYK.to_csv('final_df_NBA_NYK.csv', index=False)
...
for year in range(2012, 2024):
    # Set the URL to scrape for the current year
    url = f' https://www.hockey-reference.com/teams/NYR/{year}_games.html'
    ...
    # Add the DataFrame for the current year to the list of all data
    all_data_NHL_NYR.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NHL_NYR = pd.concat(all_data_NHL_NYR, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NHL_NYR.head())

#export final_df to csv
final_df_NHL_NYR.to_csv('final_df_NHL_NYR.csv', index=False)

# Create Merged df to show occassions that both teams played
merged_df = pd.merge(final_df_NHL_NYR, final_df_NBA_NYK, on='Date', how='inner')
...
result_df.to_csv('merged_results_NYR.csv', index=False)
