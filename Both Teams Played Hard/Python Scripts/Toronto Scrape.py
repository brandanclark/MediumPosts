...
City: Toronto (Raptors, Maple Leafs)
@author: brandanclark
"""
...
for year in range(2012, 2024):
    ...
    url = f' https://www.basketball-reference.com/teams/TOR/{year}_games.html'
    ...
    all_data_NBA_TOR.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NBA_TOR = pd.concat(all_data_NBA_TOR, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NBA_TOR.head())

#export final_df to csv
final_df_NBA_TOR.to_csv('final_df_NBA_TOR.csv', index=False)
...
for year in range(2012, 2024):
    # Set the URL to scrape for the current year
    url = f' https://www.hockey-reference.com/teams/TOR/{year}_games.html'
    ...
    # Add the DataFrame for the current year to the list of all data
    all_data_NHL_TOR.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NHL_TOR = pd.concat(all_data_NHL_TOR, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NHL_TOR.head())

#export final_df to csv
final_df_NHL_TOR.to_csv('final_df_NHL_TOR.csv', index=False)

# Create Merged df to show occassions that both teams played
merged_df = pd.merge(final_df_NHL_TOR, final_df_NBA_TOR, on='Date', how='inner')
...
result_df.to_csv('merged_results_TOR.csv', index=False)
