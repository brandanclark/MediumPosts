...
for year in range(2012, 2024):
    ...
    url = f' https://www.basketball-reference.com/teams/LAL/{year}_games.html'
    ...
    all_data_NBA_LAL.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NBA_LAL = pd.concat(all_data_NBA_LAL, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NBA_LAL.head())

#export final_df to csv
final_df_NBA_LAL.to_csv('final_df_NBA_LAL.csv', index=False)
...
merged_df = pd.merge(final_df_NHL_LAK, final_df_NBA_LAL, on='Date', how='inner')
...
result_df.to_csv('merged_results_LAL.csv', index=False)
