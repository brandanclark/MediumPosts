...
for year in range(2012, 2024):
    ...
    url = f' https://www.basketball-reference.com/teams/BOS/{year}_games.html'
    ...
    all_data_NBA_BOS.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NBA_BOS = pd.concat(all_data_NBA_BOS, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NBA_BOS.head())

#export final_df to csv
final_df_NBA_BOS.to_csv('final_df_NBA_BOS.csv', index=False)
...
for year in range(2012, 2024):
    ...
    # Add the DataFrame for the current year to the list of all data
    all_data_NHL_BOS.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NHL_BOS = pd.concat(all_data_NHL_BOS, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NHL_BOS.head())

#export final_df to csv
final_df_NHL_BOS.to_csv('final_df_NHL_BOS.csv', index=False)

# Create Merged df to show occassions that both teams played
merged_df = pd.merge(final_df_NHL_BOS, final_df_NBA_BOS, on='Date', how='inner')
...
result_df.to_csv('merged_results_BOS.csv', index=False)
