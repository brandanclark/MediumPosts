...
City: MMinnesota (Timberwolves, Wild)
@author: brandanclark
"""
...
for year in range(2012, 2024):
    ...
    url = f' https://www.basketball-reference.com/teams/MIN/{year}_games.html'
    ...
    all_data_NBA_MIN.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NBA_MIN = pd.concat(all_data_NBA_MIN, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NBA_MIN.head())

#export final_df to csv
final_df_NBA_MIN.to_csv('final_df_NBA_MIN.csv', index=False)
...
for year in range(2012, 2024):
    # Set the URL to scrape for the current year
    url = f' https://www.hockey-reference.com/teams/MIN/{year}_games.html'
    ...
    # Add the DataFrame for the current year to the list of all data
    all_data_NHL_MIN.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NHL_MIN = pd.concat(all_data_NHL_MIN, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NHL_MIN.head())

#export final_df to csv
final_df_NHL_MIN.to_csv('final_df_NHL_MIN.csv', index=False)

# Create Merged df to show occassions that both teams played
merged_df = pd.merge(final_df_NHL_MIN, final_df_NBA_MIN, on='Date', how='inner')
...
result_df.to_csv('merged_results_MIN.csv', index=False)
