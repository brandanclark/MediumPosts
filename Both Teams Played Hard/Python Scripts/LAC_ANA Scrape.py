...
City: Los Angeles Clippers, Ducks]
@author: brandanclark
"""
...
for year in range(2012, 2024):
    # Set the URL to scrape for the current year
    url = f' https://www.hockey-reference.com/teams/ANA/{year}_games.html'
    ...
    # Add the DataFrame for the current year to the list of all data
    all_data_NHL_ANA.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NHL_ANA = pd.concat(all_data_NHL_ANA, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NHL_ANA.head())

#export final_df to csv
final_df_NHL_ANA.to_csv('final_df_NHL_ANA.csv', index=False)

# Create Merged df to show occassions that both teams played
merged_df = pd.merge(final_df_NHL_ANA, final_df_NBA_LAC, on='Date', how='inner')
