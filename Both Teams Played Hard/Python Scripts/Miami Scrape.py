...
for year in range(2012, 2024):
    ...
    url = f' https://www.basketball-reference.com/teams/MIA/{year}_games.html'
    ...
    all_data_NBA_MIA.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NBA_MIA = pd.concat(all_data_NBA_MIA, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NBA_MIA.head())

#export final_df to csv
final_df_NBA_MIA.to_csv('final_df_NBA_MIA.csv', index=False)
...
for year in range(2012, 2024):
    # Set the URL to scrape for the current year
    url = f' https://www.hockey-reference.com/teams/FLA/{year}_games.html'
    ...
    # Add the DataFrame for the current year to the list of all data
    all_data_NHL_FLA.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NHL_FLA = pd.concat(all_data_NHL_FLA, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NHL_FLA.head())

#export final_df to csv
final_df_NHL_FLA.to_csv('final_df_NHL_FLA.csv', index=False)

# Create Merged df to show occassions that both teams played
merged_df = pd.merge(final_df_NHL_FLA, final_df_NBA_MIA, on='Date', how='inner')
...
result_df.to_csv('merged_results_MIA.csv', index=False)
