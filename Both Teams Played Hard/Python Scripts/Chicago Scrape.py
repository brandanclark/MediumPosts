...
for year in range(2012, 2024):
    ...
    url = f' https://www.basketball-reference.com/teams/CHI/{year}_games.html'
    ...
    all_data_NBA_CHI.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NBA_CHI = pd.concat(all_data_NBA_CHI, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NBA_CHI.head())

#export final_df to csv
final_df_NBA_CHI.to_csv('final_df_NBA_CHI.csv', index=False)

# Run the same code but for the Bruins
...
for year in range(2012, 2024):
    # Set the URL to scrape for the current year
    url = f' https://www.hockey-reference.com/teams/CHI/{year}_games.html'
    ...
    # Add the DataFrame for the current year to the list of all data
    all_data_NHL_CHI.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NHL_CHI = pd.concat(all_data_NHL_CHI, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NHL_CHI.head())

#export final_df to csv
final_df_NHL_CHI.to_csv('final_df_NHL_CHI.csv', index=False)

# Create Merged df to show occassions that both teams played
merged_df = pd.merge(final_df_NHL_CHI, final_df_NBA_CHI, on='Date', how='inner')
...
result_df.to_csv('merged_results_CHI.csv', index=False)
