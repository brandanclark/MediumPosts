...
City: San Francisco [Warriors, Sharks]
@author: brandanclark
"""
...
for year in range(2012, 2024):
    ...
    url = f' https://www.basketball-reference.com/teams/GSW/{year}_games.html'
    ...
    all_data_NBA_GSW.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NBA_GSW = pd.concat(all_data_NBA_GSW, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NBA_GSW.head())

#export final_df to csv
final_df_NBA_GSW.to_csv('final_df_NBA_GSW.csv', index=False)
...
for year in range(2012, 2024):
    # Set the URL to scrape for the current year
    url = f' https://www.hockey-reference.com/teams/SJS/{year}_games.html'
    ...
    # Add the DataFrame for the current year to the list of all data
    all_data_NHL_SJS.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NHL_SJS = pd.concat(all_data_NHL_SJS, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NHL_SJS.head())

#export final_df to csv
final_df_NHL_SJS.to_csv('final_df_NHL_SJS.csv', index=False)

# Create Merged df to show occassions that both teams played
merged_df = pd.merge(final_df_NHL_SJS, final_df_NBA_GSW, on='Date', how='inner')
...
result_df.to_csv('merged_results_GSW.csv', index=False)
