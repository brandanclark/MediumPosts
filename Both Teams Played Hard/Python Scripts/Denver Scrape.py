...
for year in range(2012, 2024):
    ...
    all_data_NBA_DEN.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NBA_DEN = pd.concat(all_data_NBA_DEN, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NBA_DEN.head())

#export final_df to csv
final_df_NBA_DEN.to_csv('final_df_NBA_DEN.csv', index=False)
...
for year in range(2012, 2024):
    ...
    # Add the DataFrame for the current year to the list of all data
    all_data_NHL_COL.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NHL_COL = pd.concat(all_data_NHL_COL, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NHL_COL.head())

#export final_df to csv
final_df_NHL_COL.to_csv('final_df_NHL_COL.csv', index=False)

# Create Merged df to show occassions that both teams played
merged_df = pd.merge(final_df_NHL_COL, final_df_NBA_DEN, on='Date', how='inner')
...
result_df.to_csv('merged_results_DEN.csv', index=False)
