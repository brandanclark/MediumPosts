...
City: New York (Nets, Isles)
Edited BKR year to remove 2012 season when team was still in New Jersey
@author: brandanclark
"""
...
for year in range(2013, 2024):
    ...
    all_data_NBA_BRK.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NBA_BRK = pd.concat(all_data_NBA_BRK, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NBA_BRK.head())

#export final_df to csv
final_df_NBA_BRK.to_csv('final_df_NBA_BRK.csv', index=False)
...
for year in range(2012, 2024):
    # Set the URL to scrape for the current year
    url = f' https://www.hockey-reference.com/teams/NYI/{year}_games.html'
    ...
    # Add the DataFrame for the current year to the list of all data
    all_data_NHL_NYI.append(df)

# Combine all DataFrames into a single DataFrame
final_df_NHL_NYI = pd.concat(all_data_NHL_NYI, ignore_index=True)

# Print the first few rows of the final DataFrame
print(final_df_NHL_NYI.head())

#export final_df to csv
final_df_NHL_NYI.to_csv('final_df_NHL_NYI.csv', index=False)

# Create Merged df to show occassions that both teams played
merged_df = pd.merge(final_df_NHL_NYI, final_df_NBA_BRK, on='Date', how='inner')
...
result_df.to_csv('merged_results_NYI.csv', index=False)
